...
class GKT_PON_ra(BaseModule, metaclass=ABCMeta):
    def __init__(self, use_light=False, downsample=32,  in_channels=768, bev_feature_channels=64, ogfH=1024, ogfW=1024,
        ...
        super(GKT_PON_ra, self).__init__()
    ...
    def forward(self, feature_maps, IE):
        I,E = IE
        feature_gkt = self.GKT(feature_maps,I,E)
        ...
        for i, fmap in enumerate(feature_maps):
            ...
            scale = 8 * 2 ** i
            featureshape.append(fmap.size())
            calib_downsamp = I.clone()
            calib_downsamp[:, :2] = I[:, :2] / scale
        ...
        feature_gkt = F.interpolate(feature_gkt, size=(98, 100), mode='bilinear', align_corners=True)
        # print('bev_feats_pon',bev_feats_pon.shape)
        # print('feature_gkt',feature_gkt.shape)
        feature_final = bev_feats_pon + feature_gkt
        return feature_final,feature_final,feature_final
