...
class DGKT(BaseModule, metaclass=ABCMeta):
    def __init__(self, in_channels=768, channels=64, resolution= 0.25 * reduce(mul, [1, 2]),
                extents=[-25.0, 1.0, 25.0, 50.0], ymin=-2, ymax=4, focal_length=630.0,
    ...
    # ymin=-2,ymax=4
        ...
        self.GKT = GeometryKernelEncoder()
        # dim: ${model.encoder.dim}
        # blocks: [128, 128, 64]
        self.GKTdecoder = Decoder(64, [128, 128, 64])
    ...
    def forward(self, feature_maps, IE):
        ...
        for i, feature_map in enumerate(feature_maps):
            ...
            combined_feature = torch.cat((feature_map, feature), dim=1)
            feature = F.interpolate(feature, size=(32,32), mode='bilinear', align_corners=False)

            DGKT_features.append(combined_feature)
        ...
        feature_gkt = self.GKT(DGKT_features,I,E)
        feature_gkt = self.GKTdecoder(feature_gkt)
        # pyva part
        # bev_feats = list()
        # scale = 8,16,32,64,128
        # calib.shape = [bs,3,3]
        # for i, fmap in enumerate(feature_maps[3]):
        #     print('test',feature_maps[3].size)
        ...
        # print(feature_gkt.size(),feature_lss.size() )
        feature_gkt = F.interpolate(feature_gkt, size=(98,100), mode='bilinear', align_corners=False)

        feature_final  = feature_gkt + feature_ls_list[0] + feature_ls_list[1] + feature_ls_list[2] + feature_ls_list[3]

        return feature_final,feature_gkt,feature_lss
