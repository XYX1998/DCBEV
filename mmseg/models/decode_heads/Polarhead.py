...
class Polarhead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,num_classes,align_corners=True, **kwargs ):
        super(Polarhead, self).__init__(**kwargs)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        # test = gt_semantic_seg[0][0]
        # print(test.shape)
        # test = test.cpu().numpy()
        # test=test[:3,:,:]
        # gt_semantic_seg_example = np.random.rand(15, 196, 200)  # 随机生成的数据

        # 将张量转换为类别索引图
        # category_map = np.argmax(test, axis=0)

        # 创建一个颜色映射表，每个类别对应一种颜色
        # num_classes = 15
        # colors = [(np.random.rand(), np.random.rand(), np.random.rand()) for _ in range(num_classes)]
        # cmap = ListedColormap(colors)

        # 使用matplotlib显示类别索引图并保存
        # plt.figure(figsize=(10, 10))  # 设置图形大小
        # plt.imshow(category_map, cmap=cmap)
        # plt.title('Semantic Segmentation Visualization')
        # plt.axis('off')  # 不显示坐标轴

        # 保存图像到磁盘
        # output_path = 'segmentation.png'
        # plt.savefig(output_path, bbox_inches='tight', pad_inches=0)
        # # selected_sample = gt_semantic_seg[:3]  # shape: [3, 196, 200]
        
        # # 将非零像素值设置为 255
        # test[test > 0] = 255
        
        # 保存图像
        # save_image(test, '/home/XYX/HFT-main/HFT/mmseg/models/decode_heads/SETS.jpg')
